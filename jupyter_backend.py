...
class JupyterKernel:
    def __init__(self, work_dir, worker_language):
        self.worker_language = worker_language
        if (worker_language.lower()=='r'):
            self.kernel_manager, self.kernel_client = jupyter_client.manager.start_new_kernel(kernel_name='ir')
    ...
    def _create_work_dir(self):
        ...
        if (self.worker_language.lower()=='r'):
            # set work dir in R environment
            init_code = f"if (!dir.exists('{self.work_dir}')) {{\n" \
                        f"    dir.create('{self.work_dir}')\n" \
                        f"}}\n" \
                        f"setwd('{self.work_dir}')"
        else:
            # set work dir in python environment
